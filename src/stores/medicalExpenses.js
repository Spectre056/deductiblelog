...
export const useMedicalExpensesStore = defineStore('medicalExpenses', () => {
	const expenses   = ref([])
	...
	async function fetchYear(year) {
		...
		try {
			const { data } = await axios.get(
				generateUrl('/apps/deductiblelog/api/medical-expenses'),
				{ params: { tax_year: year } },
			)
			expenses.value  = data.data
			yearTotal.value = data.total
		} catch (e) {
			error.value = e?.response?.data?.message ?? 'Failed to load medical expenses'
		} finally {
			loading.value = false
		}
	}
	...
	async function update(id, payload) {
		const { data } = await axios.put(
			generateUrl(`/apps/deductiblelog/api/medical-expenses/${id}`),
			payload,
		)
	...
	async function remove(id) {
		...
		await axios.delete(generateUrl(`/apps/deductiblelog/api/medical-expenses/${id}`))
