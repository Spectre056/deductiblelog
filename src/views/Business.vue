<template>
	<div class="dl-view">
		...
		<NcEmptyContent
			...
			name="No business expenses"
			:description="`No business expenses recorded for ${store.activeYear}.`"
		>
			<template #icon><BriefcaseIcon :size="20" /></template>
		</NcEmptyContent>
		...
		<NcDialog
			v-if="showDialog"
			:name="editTarget ? 'Edit Business Expense' : 'Add Business Expense'"
			:open="showDialog"
		...
		>
			<div class="dl-form">
				...
				<NcTextField
					...
					label="Description *"
					placeholder="e.g. Medical textbook, conference registration"
					:error="!!formErrors.description"
					:helper-text="formErrors.description"
				/>
				...
				<div class="dl-form-row">
					<div class="dl-field-group dl-field-grow">
						...
						<select v-model="form.category" class="dl-select">
							...
							<option v-for="c in BUSINESS_CATEGORIES" :key="c" :value="c">{{ c }}</option>
						</select>
					</div>
					<div class="dl-field-group dl-field-grow">
						<NcTextField
		...
		>
			<p>
				Delete <strong>{{ deleteTarget?.description }}</strong>
				(<strong>{{ formatAmount(deleteTarget?.amount) }}</strong>)
