...
class CashDonationMapper extends QBMapper {

    public function __construct(IDBConnection $db) {
        parent::__construct($db, 'deductiblelog_cash_donations', CashDonation::class);
    }
    ...
    public function findById(int $id, string $userId): CashDonation {
        $qb = $this->db->getQueryBuilder();
        $qb->select('*')
           ->from($this->getTableName())
    ...
    /** Sum of all donation amounts for a user + year, returned as string. */
